...
| 属性 | 内容 |
|------|------|
| 文档版本 | v2.1（2026-05-22：触发 \(C<S+L\)、可售以平台拉单为准、\(T\) 含 \(L\)） |
| 编写日期 | 2026-05-22 |
| 文档状态 | 待评审 / 可供二次加工 |
| 所属需求 | 商超 3P · WFS 库存共享 |
| 关联目录 | `../方案/01`、`../方案/02`、`../方案/03` |
| **易懂版 v2（推荐）** | [方案 v2 MD](./WFS共享库存与自动借货-方案-v2.md) · [方案说明 v2 HTML](./WFS共享库存与自动借货-方案说明-v2.html) |
| 图形化原型 v1 | [WFS共享库存与自动借货-方案说明.html](./WFS共享库存与自动借货-方案说明.html) |
...
在 **WM / BBY / Lowes** 共用 **同一 WFS 实物池** 的前提下，ERP 按 **WFS 实查** 与 **订单锁定库存** 算出各店 **应有可售**；**拉单时同步平台可售**，用 **本批销量** 作锁定 \(L_i\)；当 **\(C_i < S_i + L_i\)** 时 **一次虚拟配平** 并回写平台（**\(T_i = \mathrm{round}(PR_i)+S_i+L_i\)**）；**WFS/谷仓二选一**；**不做** 真调拨。
...
```text
              ...
              再扣「安全库存合计」Σ S_i（各店保底，配置）
                    ...
                    └──────────────────┬──────────────────┘
                                       ▼
              各店「应有」T_i = P×比例_i + 安全库存_i + 本批销量锁定 L_i
              各店「当前可售」C_i（拉单时从平台同步；平台卖出即减少）
```
...
**关键**：\(L\) 在算 **P** 之前从 \(Q_{wfs}\) 扣掉；**触发**用 \(C_i < S_i + L_i\)（\(L_i\) 为本批销量锁定）；**应有** \(T_i\) 含 \(L_i\)；\(C_i\) 以**平台拉单同步值**为准，不与 \(L_i\) 在触发式中重复「进单再减一次」。

### 0.4 订单锁定库存 —— 与 BBY/Lowes 拉单节奏（v2.1 已定）

**是什么（业务确认）**：

- **当前可售 \(C_i\)**：店铺后台/平台可售；**顾客下单当场减少**；ERP 在 **定时拉单** 时读取并写入 ERP（已是本段销量扣减后的值）。  
- **订单锁定库存 \(L_i\)**（拉单时）：**本 10 分钟销量**（拉单订单件数合计，或与上次 ERP 可售差核对，例如上次同步 100、本次读到 80 → 本批 \(L=20\)）。  
- **待拉单预占 \(L^{proxy}_i\)**：**上一轮 10 分钟销量**（拉单前 ERP 无本批明细时占池用，批末 \(L^{proxy}:=\) 本批 \(L\)）。

**确认示例（BBY）**：安全 \(S=70\)；本次拉单可售 \(C=80\)；本批锁定 \(L=20\) → \(80 < 70+20\) → **触发配平**。

**数据可见性（重要，避免误解图示）**：

- ERP **不能**实时感知 BBY 平台上下单；**只有定时拉单任务执行时**，才知道**上一个拉单周期内**的订单 **合计件数**（如本批共 25 件）。  
- **不存在** 10:03、10:07 等「逐笔进 ERP」的线上时点（那是示意拆分，**系统拿不到**）。  
- 在一个拉单周期内（约 10 分钟）：平台侧持续成交 → ERP 侧仅用 **待拉单预占** \(L^{proxy}\) 占池；**下一批拉单**才用真实件数更新 \(L^{erp}\)。  
- **待拉单预占（已定，v2.1）**：\(L^{proxy}_i\) **严格等于**上一批该店该 SKU **10 分钟销量**（与上轮拉单写入的 \(L_i\) 相同，\(\alpha=1.0\)；上批为 0 时见 §23.3.1 冷启动）。

**拉单事实（须先建立共识）**：BBY / Lowes 侧订单 **不是实时进 ERP**；ERP 约 **每 10 分钟从店铺拉一次订单**，拉进后再推 WFS。因此存在两段窗口：

| 窗口 | 含义 | 锁定能否作用 |
|------|------|--------------|
| **A · 拉单前** | BBY 平台已成交，ERP **尚未拉到单** | **不能**（ERP 还不知道这笔单） |
| **B · 进 ERP～推 WFS** | 已进 ERP，尚未推 WFS 成功 | **能**（本字段主要覆盖此处） |

**为什么要有锁定**：窗口 B 内若不锁定，WM 等店仍可按旧可售卖货。

**拉单任务内（v2.1）**：

1. 从平台/店铺读取 **当前可售** → 写入 ERP 的 \(C_i\)。  
2. 统计本批订单件数（或可售差）→ **本批销量锁定** \(L_i\)。  
3. 若 **\(C_i < S_i + L_i\)** → 触发配平；算 \(T_i = \mathrm{round}(PR_i)+S_i+L_i\)。  
4. 批末 **\(L^{proxy}_i := L_i\)**，供下一盲区占池。

扣池与展示（实现可分列）：

\[
L_i^{\text{展示}} = L_i^{\text{本批}},\quad
Q_{eff} = Q_{wfs} - \sum_i (L_i + L^{proxy}_i\ \text{或按任务态取其一})
\]

（拉单瞬间以本批 \(L_i\) 参与触发；全池扣减规则见 §9.2，避免重复扣减见 §17。）

| 时点 | 当前可售 \(C_i\) | 订单锁定 \(L_i\) | 说明 |
|------|------------------|------------------|------|
| 平台成交 | 平台侧 **当场减少** | — | ERP 可能尚未感知 |
| **定时拉单** | **同步平台可读数** | **:= 本批销量** | 触发判断 \(C<S+L\) |
| 批末 | 已更新 | \(L^{proxy}:=L_i\) | 下轮盲区 |
| 订单取消 | 回滚平台/ERP | 回滚本批统计 | — |

**WM**：拉单/推单更频时，\(L_i\) 常较小；触发式相同。

**与分配池**：每次算 \(P\) 都用 **当时的** \(Q_{wfs}\) 和 **当时的** \(L\)，即 \(Q_{eff}=Q_{wfs}-L\)。

#### 0.4.1 BBY 示例 · 每个时刻在发生什么（图文）

> **图形版（推荐先读）**：[WFS共享库存与自动借货-方案说明.html](./WFS共享库存与自动借货-方案说明.html) 导航 **「10分钟时刻详解」**；改 §7 演算参数后，卡片与下表数字联动。

以下以 **BBY 3P-US、单一 8 位 SKU**、上一批 **推 WFS 成功 25 件** 为例（数字可与演算表不一致，逻辑相同）。

| 时刻 | ERP 能否感知本段新单 | 平台侧（BBY） | ERP / 系统动作 | 待推 WFS \(L^{erp}\) | 待拉单预占 \(L^{proxy}\) | 锁定合计 \(L\) | 说明 |
|------|----------------------|---------------|----------------|----------------------|--------------------------|----------------|------|
| **09:50～09:59** | **否** | 顾客持续下单；展示库存按**上次同步**扣减 | 无拉单、无推 WFS；仅用预占占池 | 0 | 25 | **25** | 25 = 上一批 10:00 推成功件数；**不是**实时订单 |
| **10:00:00** | 即将 **是** | 同上 | 定时任务启动 | 0 | 25 | 25 | 任务开始，尚未拉单 |
| **10:00 拉单完成** | **是（本批合计）** | 09:50～10:00 成交**一次性**进 ERP | 本批 +25 件 → \(L^{erp}\) 增加 | **25** | **0** | **25** | 拉单后不再用代理；已知本批总量 |
| **10:00 推 WFS 成功** | 是 | — | 按回执释放 \(L^{erp}\) | **0** | 0 | **0** | 成功 −25（可与拉单数不同，以推单为准） |
| **10:00 批末** | **否**（下一窗口） | 10:00 后又可成交，ERP 仍无明细 | \(L^{proxy} :=\) 本批推成功 **25** | 0 | **25** | **25** | 进入下一盲区；α=1.0 严格相等 |
| **现在（例 10:05）** | **否**（等 10:10） | 新成交 ERP 仍不知件数 | 仅预占 + 若有「已进 ERP 未推」则 \(L^{erp}\) | §7 可调，例 **5** | **25** | **30** | **无** 10:03/10:07 台账行 |
| **10:10 拉单** | **是（新一批合计）** | — | 才知 10:00～10:10 本批总量；再推 WFS、再刷新预占 | 循环 | 循环 | 循环 | 不以中间时刻逐笔建账 |

**读表要点**：

1. **黄色盲区**（09:50～10:00、10:00～10:10）：ERP 只有 \(L^{proxy}\)，没有本段成交的逐笔/合计。  
2. **绿色任务条**（10:00 一刻）：拉单 → \(L^{erp}\) ↑ → 推 WFS → \(L^{erp}\) ↓ → 批末写 \(L^{proxy}\)。  
3. **「现在」行**：若 §7 中「待推 WFS」&gt; 0，表示已有订单进 ERP 尚未推完（与是否到 10:10 无关）；若已为 0，表示 10:00 那批已全部推完，仅剩预占 25。
...
并行：拉单任务内更新 \(C\)、\(L\)；推 WFS 按 §13 履约（**不以「进 ERP 再减 C」作为唯一扣减路径**）。

### 0.6 自动配平（虚拟借货）在流程中的位置

**触发（v2.1 已定）**：任意一店 **\(C_i < S_i + L_i\)**（\(C_i\) = 拉单同步的平台可售；\(L_i\) = 本批销量锁定）。
...
### 0.6.1 配平后仍全店低于安全 → 关子店、关共享（硬性处置）

**触发**（一次虚拟配平**之后**立即判断）：

- **全部参与店**仍满足 **`C_i < S_i + L_i`**（或配平后仍无法达到 \(T_i\)，通常表示 WFS 实物池已耗尽）；  
- 与「仅单店低于安全 → 正常配平 + 告警」区分：本规则是 **极端池耗尽** 场景。

**自动处置**（**2026-05-22 业务确认**）：

| 对象 | 动作 |
|------|------|
| **BBY、Lowes**（及扩展的非 WM 参与店） | 平台可售同步 **0**（停售） |
| **WM 3P-US** | **不** 将平台可售置 0（沃尔玛侧仍按既有逻辑展示/售卖，以实现为准） |
| **该 8 位 SKU · WFS 仓共享** | **整 SKU 停用** WFS 仓共享（共享开关关闭）；WM 与 BBY/Lowes **均不再共用** 本方案的共享池与自动配平 |
| **告警** | 群消息：共享已自动关闭 + 子店已停售（WM 未强制置 0） |

**恢复**：运营人工复核 WFS 实查、补货或调整比例后，**重新启用** 该 SKU 的 WFS 仓共享并配置可售（本方案不自动恢复）。
...
|---------|------------|
| 当前 | \(C_i\)，拉单时从平台同步 |
| **订单锁定库存** | 本批 \(L_i\)（销量）；列可拆：本批锁定 / 待拉单预占 / 扣池合计 |
| 安全 | \(S_i\)，配置固定件数 |
| 触发 | \(C_i < S_i + L_i\) → 是 |
...
**时刻 T1（10:00 定时拉单批处理）**：本批从 BBY **一次性拉到** 30 件进 ERP → \(L^{erp}_{bby}=30\)（推 WFS 前），推成功后释放；批末 \(L^{proxy}_{bby}:=30\)。  
→ 在 **10:00～10:10** 之间 ERP **不知道** 新成交明细，仅展示预占 30。  
→ \(L=30\)，\(Q_{eff}=170\)，\(P=70\) → 应有降至约 51/51/68。
...
| 应有库存 | 配平目标：`可分配池 × 共享比例 + 安全库存 + 本批销量锁定`（v2.1，见 §9.3） |
...
```
∃ 参与店 i ： 当前逻辑可售_i < 安全库存_i + 订单锁定库存_i
```

（v2.1：**订单锁定库存** = 本批 10 分钟销量，在 **拉单任务** 内与平台可售一并更新。）

**当前逻辑可售** 定义：

- **拉单时**从店铺/平台同步的可售（平台卖出即减少）；
- 与 ERP 展示列一致；回写延迟时以 ERP 已同步值为准。
...
\[
T_i = \lfloor P \times R_i \rfloor + S_i + L_i \quad \text{（或四舍五入，需统一舍入规则；\(L_i\) 为本批销量锁定，v2.1）}
\]
...
| \(O_{pending}\) | \(\sum_i (L^{erp}_i + L^{proxy}_i)\)：待推 WFS 真实锁定 + **待拉单预占**（§23.3.1） |
...
与 **§24** 同一能力；列表列 **「订单锁定库存」** = \(L_i = L^{erp}_i + L^{proxy}_i\)（可对运营展示子项明细）。

| 子项 | 窗口 | 规则 |
|------|------|------|
| \(L^{erp}_i\) | B · 已进 ERP | 进单 **+实际件数**；推 WFS 成功 **−实际推单件数** |
| \(L^{proxy}_i\) | A · 未进 ERP | 见 §23.3.1 |

- 可分配池：\(Q_{eff} = Q_{wfs} - \sum_i L_i - \sum_i S_i\)（§9.2）。

#### 23.3.1 待拉单预占：上轮批量件数代理（P0 · 无实时订单时推荐）

**动机**：ERP 约每 10 分钟才从 BBY/Lowes **拉单**，拉单前无法对真实订单做 \(L^{erp}\)。在「相邻两批销量接近」假设下，用**上一批周期内的订单件数**外推下一窗口，填补窗口 A。

**取值**（**已定**：2026-05-22）：

```text
last_batch_qty_i = 上一批定时任务中，该店该 SKU「推 WFS 成功」件数合计
L^proxy_i := last_batch_qty_i    // 严格相等；α=1.0；下一窗口预占仅由此刷新，不用本窗口猜数
```
...
|------|------|
| 待推 WFS 锁定 | \(L^{erp}_i\) |
| 待拉单预占 | \(L^{proxy}_i\)（说明：上批推 WFS 成功 {last_batch_qty} 件） |
| 订单锁定库存 | \(L_i = L^{erp}_i + L^{proxy}_i\) |

**合理性**：**有条件成立** — 平稳 SKU、固定拉单周期下误差可接受；**不单独依赖**于大促脉冲、上批为 0 冷启动、爆款上新。详见 [BBY拉单延迟与超卖优化-同步说明.md](../BBY拉单延迟与超卖优化-同步说明.md) §3.0。

**边界**：

| 场景 | 处理 |
|------|------|
| `last_batch_qty = 0` | 用近 3 批 EMA 或 7 日同时段均速；仍为 0 则 \(L^{proxy}=0\) 并依赖超卖扫描 |
| 本批实际 ≫ 上批 | 允许短暂低估 → L2/L3 压子店可售 + 告警 |
| 与 \(L^{erp}\) | **禁止同一笔订单既在 \(L^{erp}\) 又在 \(L^{proxy}\)**；仅「未进 ERP」部分用 proxy |
| 有 Webhook 实时进单 | \(L^{proxy}\) 可置 0 或仅兜底 |

**界面**：主列仍显示合计 \(L_i\)；Tooltip/展开：`待推WFS {L^erp} + 待拉单预占 {L^proxy}（上批 {last_batch_qty}）`。
...
| v2.2 | 2026-05-22 | §0.4 区分拉单前窗口 A / 进 ERP 后窗口 B；链至《BBY拉单延迟与超卖优化-同步说明》 |
| v2.3 | 2026-05-22 | §23.3.1 待拉单预占：上批件数代理 \(L^{proxy}\)；\(L=L^{erp}+L^{proxy}\) |
| v2.4 | 2026-05-22 | 已定：上批=**推 WFS 成功**件数；α=1.0；列表三列展示 |
| v2.5 | 2026-05-22 | §0.4 明确仅拉单批可知订单合计；§0.6.1 配平后全店仍低于安全→子店0+关共享 |
| v2.6 | 2026-05-22 | §0.6.1 确认：WM 不置0、整 SKU 关 WFS 共享；L^proxy 严格=上批推 WFS 成功 |
