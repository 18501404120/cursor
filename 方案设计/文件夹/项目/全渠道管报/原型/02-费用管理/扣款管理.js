(function () {
  ...
  function ratioDisplay(row) {
    ...
    if (isDeptFixedFee(row)) return '部门比例';
  ...
  function ratioCalcButton(row) {
    if (isDeptFixedFee(row)) {
      return '<button type="button" class="calc-link" data-action="calc" data-field="ratio" data-id="' + esc(row.id) + '" title="查看部门计提详情（只读）">部门比例</button>';
    }
  ...
  function init() {
    if (!store || typeof store.getDeductions !== 'function') {
