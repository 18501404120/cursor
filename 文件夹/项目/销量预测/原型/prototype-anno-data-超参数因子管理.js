  ...
  var ANNOS = [
    ...
    },
    {
      ...
      html:
        ...
        "<p><strong>【逻辑 · 起止未填全】</strong>若某行因子<strong>尚未填完整开始/结束日期</strong>，可不参与「与默认窗求交集」过滤（仍可按编号、事件、状态被搜出），方便先建草稿；正式环境是否允许以<strong>产品/后端开关</strong>为准。</p>" +
    ...
    },
    {
      ...
      html:
        ...
        "<p><strong>【逻辑】</strong>新增行<strong>不设默认值</strong>：除系统分配的新编号外，事件说明、因子值、起止日期均为空，由用户填写后点行内「保存」。未保存前点「取消」丢弃本行编辑。保存时应校验：系数为数值、起止合法、起止与默认窗是否有交集（产品可配置是否允许保存落在窗外的因子）。</p>" +
    ...
    },
    {
      ...
      title: "按钮：失效（批量）",
      html:
        "<p><strong>【基础】</strong>将勾选记录从「生效」批量改回「草稿」类不可用态，用于下线政策或纠错；原型中<strong>仅产品经理</strong>可点（权限模拟）。</p>" +
        "<p><strong>【逻辑 · 前置】</strong>所选须<strong>全部为「生效」</strong>，否则整批拒绝并提示（与「生效」按钮要求「全部为草稿」对称）。</p>" +
        "<p><strong>【逻辑 · 绑定拦截】</strong>若任一因子<strong>已绑定到 MSKU</strong>（配置中心正在引用），则<strong>不允许失效</strong>：应提示并<strong>列出不可失效的编号</strong>，避免线上预测突然缺因子。</p>" +
        "<p><strong>【逻辑 · 成功后】</strong>通过后状态回退为草稿（或产品定义的失效态）、写<strong>审计日志</strong>（建议只记变更字段，如状态）。历史已跑过的预测是否回溯重算由<strong>数据方案</strong>另定。</p>" +
        "<p><strong>【交互】</strong>二次确认弹窗；失败时保留原状态。</p>",
    },
    {
      ...
      title: "按钮：删除（批量）",
      html:
        "<p><strong>【基础】</strong>物理删除勾选行；通常<strong>仅「草稿」</strong>可删，生效行在操作列「删除」灰置。</p>" +
        "<p><strong>【逻辑】</strong>批量时若勾选集合里<strong>混入非草稿</strong>，应<strong>整批拦截</strong>并提示，避免误删已上线因子。通过后删除行、刷新列表、写审计；编号<strong>永不回收</strong>（见编号列红点）。</p>" +
        "<p><strong>【交互】</strong>二次确认弹窗防误触。</p>",
    },
    {
    ...
    },
    {
      ...
      title: "操作日志面板",
      html:
        "<p><strong>【基础】</strong>右侧固定宽度面板，展示<strong>当前选中因子编号</strong>的变更历史。</p>" +
        "<p><strong>【逻辑 · 列】</strong>列含<strong>调整人、调整时间、调整前、调整后</strong>。<strong>只展示有变化的字段</strong>：未改动的字段不重复输出，避免刷屏；同一保存操作若改多个字段，可拆多行或合并展示（以后端为准）。</p>" +
        "<p><strong>【逻辑 · 空态】</strong>未点任何行「日志」时显示引导文案；编号切换后列表随之切换。</p>" +
