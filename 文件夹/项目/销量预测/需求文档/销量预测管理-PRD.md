...
| 文档 | 路径（相对本文件 `./` 或上级） | 主要内容 | 维护建议 |
|------|--------------------------------|----------|----------|
| **本 PRD** | `./销量预测管理-PRD.md` | 背景价值、整体方案、文档索引、**各页面职责**、**主工作台详设** | 产品主导；**交互/口径变更先改本文与原型红点，再排开发** |
| **项目背景** | `../项目背景.md` | 行业背景、经济/运营/战略价值论述 | 与市场/战略材料对齐后偶发更新 |
| **立项会要点** | `../会议/立项会要点.md` | 边界、forecast 语义、不做项 | 重大立项变更后更新 |
| **销量预测-分步计算说明** | `./销量预测-分步计算说明.md` | 系数公式、符号约定、数值示例、范围（含 **1.1 ASIN 合并**） | 算法/因子规则变更时**先改此文档**，再同步 PRD 与配置红点 |
| **R1至R3规则说明** | `./R1至R3规则说明.md` | 锁定提前量、单月/窗口达成率、**5.3 Q_pred=0 排除** | 与主工作台筛选、KPI、列表列强一致 |
| **预测销量上下限规则-终版** | `./预测销量上下限规则-终版.md` | 五类因子 min/max、悲观/乐观合成 | 主图区间带、详情上下限列强一致 |
| **钉钉同步说明** | `../钉钉同步说明.md` | 与钉钉文档同步流程 | 协作规范 |
| **主工作台原型说明** | `../原型/销量预测管理-原型说明.html` | 页面静态结构、交互 Demo | 前端/产品 |
| **主工作台红点数据** | `../原型/prototype-anno-data-销量预测管理.js` | **控件级验收说明**、导出模板链接、与 PRD 对齐声明 | 与 **第 5 章** 同步更新 |
| **预测配置中心原型** | `../原型/销量预测配置中心-原型说明.html` + `prototype-anno-data-销量预测配置中心.js` | 因子/弹性配置、列逻辑 | 因子口径变更时同步 |
| **超参数因子管理原型** | `../原型/超参数因子管理-原型说明.html` + `prototype-anno-data-超参数因子管理.js` | 因子库、编号、区间 | 超参字典变更时同步 |
...
| 卡片 | 展示内容 | 计算逻辑 | 数据来源 |
|------|----------|----------|----------|
| **R1 预测达成率** | 汇总百分比 | 在 **当前筛选结果集**（应用阈值后剩余行）上，对每行算窗口聚合（**P 取 R1**），再对多行做展示层聚合（**演示脚本为简单平均**；正式环境由产品确认是「等权平均」还是「按销量加权」并写死） | 各 MSKU 的 R1 锁定预测 + 实际关账 |
| **R2 / R3** | 同上 | **P 分别取 R2、R3** | 同上 |
...
| **实际销量** | **柱状**（非折线） | 左轴 | 财务/运营约定出库或结算口径 | 未关账月可空 |
| **R1/R2/R3 预测达成率** | 折线 | **右轴 0%～100%** | 单月公式 `100% − |A−P| / max(A,P)`（**P** 分别取 R1/R2/R3 当月锁定预测） | 未关账月断点 |
...
- **承载 MSKU** 独占非零预测；其余同源行 **点预测销量（Q_pred）= 0**。  
- **达成率**：**Q_pred=0** 的月份/行不参与达成率聚合（见 [R1至R3规则说明.md](./R1至R3规则说明.md) **§5.3**）。主工作台列表/KPI/主图须与之一致。
...
*本文档随迭代更新；口径变更须同步第 3 章所列推导文档与原型红点。*
