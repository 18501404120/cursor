...
- **视觉与现网一致**：三卡 KPI、原筛选字段、原工具栏按钮、原宽表结构（不含额外「数据来源/关联任务」列，避免信息过载）
- **增量功能最小化**：仅增加 ① 通知渠道填报 ② 渠道填报进度（折叠/抽屉）

### 5.2 筛选区

与现网对齐：年份、区域、国家、渠道/店铺、销售人员、部门、场景品类、品线、model、SKU、GTM 标签、状态。
...
- 文案：`12 / 18 店铺已提交 · 待审核 5 · 68%`
- 「查看明细」打开右侧抽屉：按店铺展示负责人、SKU 数、进度、状态
- 抽屉内可「通知未提交店铺」
...
| 按钮 | 说明 |
| --- | --- |
| **通知渠道填报** | **新增 · 主操作**：一键通知所选店铺负责人（见 5.6） |
...
| BP 版本 | 如 2025 年度 BP |
| 截止日期 | 必填 |
| 通知范围 | 按当前筛选 / 仅未提交 / 全部店铺 |
| 通知说明 | 可选，随通知下发 |

确认后：系统按店铺负责人映射发送通知；渠道侧出现待办，进入 P2 填报。

### 5.7 列表字段

与现网一致，**不增加**数据来源、关联任务列（汇总后数据与总部导入行无差别展示；进度在抽屉查看）。
