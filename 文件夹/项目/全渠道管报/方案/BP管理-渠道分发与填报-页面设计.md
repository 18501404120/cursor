...
- **顶栏**：任务信息 + 状态 + 截止日期倒计时
- **工具栏**：下载模版、导入（须符合《批量导入-全局UI规范》）、**新增 SKU**、保存草稿、**提交提报**
...
| 状态 | 数据表现 | 渠道操作 |
| --- | --- | --- |
| **待填写** | 任务内 **无 SKU 行**（指标全空，主表同步显示「—」） | **新增 SKU**（弹窗录入 SKU/场景品类/GTM 标签）或 **导入** 追加行 |
| **草稿** | 已有 SKU 行，可部分未填完 | 继续编辑、新增/删除行、保存草稿 |
| **定版** | 只读 | 查看；变更回草稿后恢复编辑 |

- 首次 **保存草稿** 或开始填写有值时，任务状态由「待填写」→「草稿」
- 删除行需二次确认；提交前须至少 1 行且全部 SKU 指标完整
