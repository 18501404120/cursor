...
- 在一个拉单周期内（约 10 分钟）：平台侧持续成交 → ERP 侧仅用 **待拉单预占** \(L^{proxy}\)（= 上批推 WFS 成功件数）估算占用；**下一批拉单**才用真实件数更新 \(L^{erp}\)。
...
| **该 SKU · WFS 仓共享** | **库存共享开关自动关闭**（配置停用，记 `SHARE_AUTO_OFF` 日志） |
| **WM** | 默认 **不** 因本规则单独关共享（沃尔玛侧占 WFS 主库存）；是否同步压可售 **待业务确认** |
| **告警** | 群消息：共享已自动关闭 + 子店已停售 |

**恢复**：运营人工复核 WFS 实查、补货或调整比例后，**重新启用** WFS 仓共享并配置可售（本方案不自动恢复）。
...
L^proxy_i := last_batch_qty_i                 // α = 1.0，不放大
```
