...
L^{proxy}_i := \text{last\_batch\_qty}_i \times \alpha\ (\alpha\ge 1\ \text{可配置})
\]
...
```text
last_batch_qty_i = 上一批定时任务中，该店该 SKU「拉单进 ERP」的件数合计
（备选：上一批「推 WFS 成功」件数；失败单需统一口径）
```

**更新**（每批拉单/推单任务结束时，按店、按 SKU）：

```text
L^proxy_i := ceil( α × last_batch_qty_i )     // α 默认 1.0，大促可 1.2～1.5
```
