...
### 0.4 订单锁定库存（预占）—— 与 BBY/Lowes 拉单节奏
...
- **待拉单预占（已定）**：下一窗口的 \(L^{proxy}_i\) **严格等于**上一批该店该 SKU **推 WFS 成功件数合计**（\(\alpha=1.0\)，不做放大、不做 EMA 替代，除非上批推成功为 0 时按 §23.3.1 冷启动规则）。
...
**窗口 A 的补偿（无实时订单时，推荐）**：用**上一批 10 分钟拉单（或推 WFS 成功）件数**作 **待拉单预占** \(L^{proxy}_i\)，假设相邻两批销量接近；与真实锁定合并：

\[
L_i = L^{erp}_i + L^{proxy}_i,\quad
L^{proxy}_i := \text{last\_batch\_pushed\_qty}_i\ (\text{上批推 WFS 成功件数},\ \alpha=1.0)
\]

进 ERP 后只增加 \(L^{erp}\)；每批任务结束后刷新 \(L^{proxy}\)。合理性、边界与 \(\alpha\) 见 [BBY拉单延迟与超卖优化-同步说明.md](../BBY拉单延迟与超卖优化-同步说明.md) §3.0；§23.3。
...
- **全部参与店**（WM、BBY、Lowes）仍满足 **`当前可售 < 安全库存`**（配平仍无法把任一家抬到安全线以上，通常表示 WFS 实物池已耗尽或缺口无法在店间调剂）；  
...
|---------|------------|
| 当前 | \(C_i\)，已扣本店锁定 |
| **订单锁定库存** | \(L_i=L^{erp}_i+L^{proxy}_i\)；列可拆：待推 WFS / 待拉单预占 / 合计 |
| 安全 | \(S_i\)，配置固定件数 |
| 触发 | \(C_i < S_i\) → 是 |
