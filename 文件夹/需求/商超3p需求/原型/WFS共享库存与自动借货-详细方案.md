...
22. [附录](#22-附录)
23. [10 分钟推单窗口超卖风险与防控（BBY / Lowes）](#23-10-分钟推单窗口超卖风险与防控bby--lowes)
...
| 待推 WFS 占用 | BBY/Lowes 已进 ERP、**尚未推 WFS 仓** 的订单数量合计（按 8 位 SKU） |
| 有效可用 | 配平/推单计算用：`WFS 实查 − 待推占用 − WM 近期占用预留`（见 §23） |
...
| M10 | 人工借货 | — | **已有，本方案不展开** |
| M11 | 订单预占（待推 WFS） | P0 | 拉单即扣减可用量，对齐 10 分钟窗口 |
...
其中 **有效 WFS 总量**（用于配平与风险判断，避免 10 分钟窗口重复卖）：

\[
Q_{wfs}^{eff} = \max\left(0,\; Q_{wfs} - O_{pending} - O_{wm\_buf} \right)
\]

| 符号 | 含义 |
|------|------|
| \(Q_{wfs}\) | WFS 接口实查 |
| \(O_{pending}\) | BBY + Lowes **待推 WFS** 订单占用（已拉单未推仓，见 §23.3） |
| \(O_{wm\_buf}\) | 可选：WM 近 **Δt**（如 10min）已成交未反映到实查的缓冲量 |

**说明**：安全库存先从 **有效总量** 中「预留」，剩余部分按比例分给各店（再加回各店安全库存形成应有）。  
**若未接入预占**，仍用 \(Q_{wfs}\) 配平，但 **必须** 启用 §23 风险扫描与自动压可售，否则超卖窗口无法收敛。
...
| 时点 | 店铺 | 建议 |
|------|------|------|
| **订单进入 ERP（拉单成功）** | BBY、Lowes | **立即预占** `待推 WFS 占用` + 扣减该店逻辑可售（见 §23.3） |
| 订单进入 ERP | WM | 按平台节奏预占或按 WM 成交流速单独缓冲 |
| 推 WFS 仓成功 | BBY、Lowes | 将占用从 `待推` 转为 `已推`；与 WFS 回传对账 |
| 推单成功 | 全店 | 二次校验逻辑可售 |
...
| pending_wfs_qty | 待推 WFS 占用（BBY/Lowes） |
| risk_flag | 最近是否超卖风险 |

### 14.3 表：wfs_share_pending_order（待推 WFS 占用明细，建议）
...
| shop_code | BBY / Lowes |
| qty | 件数 |
| status | pending_push / pushed / cancelled |
| captured_at | 进 ERP 时间 |
| pushed_at | 推 WFS 成功时间 |

### 14.4 表：wfs_share_balance_log（配平/借货）
...
| 订单预占维护 | **≤10 min** 或与拉单同频 | 汇总 \(O_{pending}\)，更新运行态 |
...
7. BBY 订单进 ERP 后、推 WFS 前：`待推占用` 增加、逻辑可售减少；推 WFS 后占用转移。
...
### 23.3 处理方式一：订单预占（P0，强烈推荐）

**规则**：BBY、Lowes 订单 **一进 ERP**（拉单成功且判定走 WFS 履约）即：

1. 增加 `wfs_share_pending_order` 明细；
2. \(O_{pending}\) += 订单行件数；
3. **立即扣减** 该店 `current_sellable`（及可选 **同步降低平台可售**，P1）；
4. 推 WFS **成功** 后：明细状态 → `pushed`，\(O_{pending}\) 减少，改记 WFS 实扣；
5. 推单 **失败** 或 **取消**：回滚预占。

**与「无货不推」关系**：

- 进 ERP 时即校验：`current_sellable ≥ 行数量`，不足则 **标记异常、不进入待推队列** 并告警。

**效果**：10 分钟内的 BBY/Lowes 新单不会再「重复占用」已在别店卖出的份额。
