...
- **边录边写磁盘**：录音按秒写入会议目录 `recording.webm`，崩溃或未点结束也不会整场丢失
- 启动时若发现未完成录音，会询问是否立即转写；也可在菜单栏点「转写未完成的录音」
...
4. **双悬浮窗**：转写较久时，结束录音后可点「打开第二个悬浮窗」，或菜单栏图标 → **新建悬浮窗（录下一场会议）**；最多同时 **2 个** 窗口，转写任务自动排队
5. 右上角 **×**：录音中 = **取消本场**（不保存）；转写中 = **隐藏**（后台继续）；待机 = 隐藏窗口
6. **完全退出** → 菜单栏麦克风图标 → **退出**（退出时会把已录内容落到磁盘，下次启动可转写）
...
新版会边录边写入会议目录里的 `recording.webm`。重新打开应用会提示「发现未完成的会议录音」，选「立即转写」即可。也可点菜单栏麦克风 → **转写未完成的录音**。
